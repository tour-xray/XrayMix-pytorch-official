...
from utils.utils import convert_annotation, get_classes, get_classes_backnolabel
...
# 定义 car 和 fore 文件夹路径
    car_folder_path = os.path.join(Out_VOCdevkit_path, "D:/xray/object-detection-augmentation/car")
    fore_folder_path = os.path.join(Out_VOCdevkit_path, "D:/xray/object-detection-augmentation/fore")

# 创建 car 和 fore 文件夹（若不存在）
    if not os.path.exists(car_folder_path):
        os.makedirs(car_folder_path)
    if not os.path.exists(fore_folder_path):
        os.makedirs(fore_folder_path)
    ...
    #如果背景没有xml文件，直接等于None
    if Origin_Annotations_path is None:
        xml_names = None
    else:
    ...
    for index in tqdm(range(Out_Num),desc='Processing'):
        ...
        if xml_names is None:
            back_path = os.listdir(Origin_JPEGImages_path)
            sample_xmls_back = sample(back_path, 1)
            sample_xmls_clean = sample(xml_name_clean, 1)
            sample_xmls = [sample_xmls_clean[0]]
            unique_labels = get_classes_backnolabel(sample_xmls, Origin_Annotations_path_clean)
            jpg_name_1  = os.path.join(Origin_JPEGImages_path, os.path.splitext(sample_xmls_back[0])[0] + '.bmp')
            jpg_name_2  = os.path.join(Origin_JPEGImages_path_clean, os.path.splitext(sample_xmls_clean[0])[0] + '.jpg')

            xml = os.path.join(Origin_Annotations_path_clean, sample_xmls_clean[0])
            width,height = Image.open(jpg_name_1).size
            back_size = [height,width]

            line_1 = convert_annotation(jpg_name_1, None, unique_labels)
            line_2 = convert_annotation(jpg_name_2, xml, unique_labels)
            ...
            # 保存 image_1 和 image_2 到对应的文件夹
            car_image_name = f'mixpl20241027clean_{index}_car.jpg'
            fore_image_name = f'mixpl20241027clean_{index}_fore.jpg'
        ...
        img.save(os.path.join(Out_JPEGImages_path, 'mixpl20241027clean_'+ str(index) + '.jpg'))
        img1 = Image.fromarray(image_1.astype(np.uint8))
        img2 = Image.fromarray(image_2.astype(np.uint8))
        img1.save(os.path.join(car_folder_path, car_image_name))
        img2.save(os.path.join(fore_folder_path, fore_image_name))

        write_xml(os.path.join(Out_Annotations_path, 'mixpl20241027clean_'+ str(index) + '.xml'), os.path.join(Out_JPEGImages_path, 'mixpl20240919clean_'+str(index) + '.jpg'), \
